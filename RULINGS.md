...
## R-047 — 2026-08-03 — The chaining paragraph, restated in the current grammar

Ruled 2026-08-03 by both editors, resolving the divergence R-046 flagged and could not itself close.

**The ratified text:**

> Chaining paragraph restated in the current grammar. R-035's ratified chaining paragraph is superseded in wording only — semantics unchanged — so the provenance page's prose speaks the one notation everywhere. The restated text applies the badge operators to every example: the chained example becomes 'AI¹ = Human – AI² (editor)' and its byline rendering follows; all other examples in the paragraph take the same operators. R-035's original wording remains in the rulings log as the record of what was ratified when. The verbatim guard transfers: it now protects the restated paragraph as the current ratified text.

**The restated paragraph, in full, as it now stands at /provenance:**

> Tiers may chain, read left to right, when a work passes through more hands. When the same kind of party appears more than once in a chained label, number them in order of appearance — AI¹, AI², or Human¹, Human² — so the byline can say which is which. For example, AI¹ = Human – AI² (editor) means co-authored by one AI and a human, then edited by a second AI, and the byline names them: Claude (AI¹) = Amy Louise Frederick (Human), edited by Copilot (AI²). Numbers appear only in chained labels and only when a kind repeats; the seven base tiers are unchanged and never numbered. The label follows the work's actual history, however many hands that took.

**Exactly one character moved.** The chained example's `+` became `–`, which is the operator R-046 gives a party that edited. The byline rendering that follows it needed nothing: it joins its parties with `=`, which was already the badge grammar, and names the editing party in words rather than with an operator. No other example in the paragraph carries an operator. Semantics are untouched at every point — the same chain, the same numbering rule, the same byline.

**Why this needed a ruling at all, recorded because the mechanism worked.** A sweep on 2026-08-03 tried to update the operator here directly and the suite refused it: the paragraph is pinned byte-for-byte as ratified text, and the assertion exists precisely so that ratified prose changes by ruling rather than by a commit. The guard did its job, the change came back as this ruling, and the path from flag to ruling is the one R-046's own carve-out clause described.

**R-035 keeps its wording, and this log keeps both.** R-035 stands unedited above, with `+` in its example, as the record of what was ratified on 2026-08-01. This ruling is what is true now. Anyone reading the two together sees an operator change and no change of meaning, which is the whole of it.

**The guard transfers rather than lapsing.** The suite now pins the restated paragraph, so the current ratified text is protected exactly as the previous one was. A ratified string that lost its guard while being restated would be the worst outcome of the three available.
...
## R-049 — 2026-08-03 — The journal's colours are its provenance colours

Ruled 2026-08-03 by both editors, across five preview walks, with the design pass that settled the palette.

**The ratified text:**

> The journal's green is #4B8E4D, and it is one colour: the badge ring that marks the AI side of the provenance spectrum and the accent that carries the journal's own name, navigation and section headings are the same value, not two values that resemble each other. The ground is a blush cream, #FAF3EF. Where type at ordinary sizes cannot carry the colour, it takes a darker stop of the same green — same hue, same saturation, lower lightness — never a different green. The semantics are the badges': green is the AI side, pink the human side, and the page is now built out of both.

**The colour was matched to an object, not chosen from a screen.** It began as the sage the badge rings already wore, moved through an apple-olive and an olive-avocado across three walks against the human editor's eyeglass frames, and was settled by holding the frames to a swatch chart. Recorded because the route matters: each intermediate candidate was a yellow-green within a few degrees of the last, and the answer was a different family altogether — hue 79° to 122°. Four passes of eyeballing converged on the wrong neighbourhood; one measurement against the physical object ended it.

**One colour, and it is enforced rather than intended.** `--accent-bright` in the stylesheet and `RING_AI` in the badge module are the same value. They cannot share a literal across CSS and JavaScript, so the suite asserts they are equal — two hex values in two languages meaning one thing being exactly the pair that drifts. The identity colour and the provenance colour are the same colour by test, which is the only way a claim like that survives a year.

**Three stops, one green.** #4B8E4D measures 3.63:1 on the ground: enough for a ring or a rule, not for type. Type takes #3E743F at 5.07:1; a filled button's hover takes #305B32 at 7.17:1. All three sit at hue 121–123° and saturation 30–31%, so they read as one colour used three ways rather than as three greens, and the suite asserts that family too. **One surface departs from this and does so deliberately:** *Support the journal* in the footer is set in the bright stop at 15.3px, below the ratio its size answers to. It is the editors' call, made on the walk, and it is flagged in the rule itself so a later reader finds the exception where the exception lives.

**The ground carries the other half of the semantics.** #FAF3EF is the old cream moved from hue 42° to 21° at the same lightness — still cream at a glance, blush when looked at. The badge ring's two colours are the AI side and the human side; the page now sets the AI side as its accent and the human side as its ground, so the provenance standard is not only illustrated on /provenance but is the material the journal is printed on.

**The badge became fixed geometry, which was a correctness fix and not a polish item.** The notation inside the ring was being moved by whatever container it landed in, because an SVG `<text>` inherits every typographic property unless it declares its own. Two faults were live: the notation rendered in italic on every article header, where `.article-byline` is italic, and one inherited `text-transform` from a neighbouring kicker would have rendered the editor superscript as a capital E — silently changing what the notation says. Every property that can move a glyph is now declared, the box is pinned in three dimensions, and the mark is verified identical inside hostile containers. **A badge that renders differently depending on where it sits is not an abbreviation of the record; it is a second thing that resembles one.** The marks are also a quarter larger at every placement, at exactly 1.25 so the two sizes stay proportional.

**A piece is named one way.** The unquoted display title now reaches every surface a reader reads — the issue contents, the cards, the archive, the answers list, the browser tab, the digest — where before only the article header had it, so the front page and the piece disagreed about the piece's own name. The recorded title is untouched and asserted untouched: the feeds, the JSON indexes, `llms.txt`, the structured data and the frontmatter all keep the quotation marks, because for the cover of Issue No. 1 the marks are part of the title. **The line is who is reading — a person or a parser** — and it is the same line the provenance fields already draw between what is displayed and what is stored.

**Letters is built like the other section pages.** Its description line was set as a 2.4rem heading, which made it the largest description line on the site and made Letters the one section page whose name was not its heading. The name and the description trade places and the terminal period is dropped. **Flagged in the same breath: every other description line on the site ends in a period**, so Letters is now the exception rather than the rule it was measured against, and uniformity either way is a separate call the editors have not made.

**Nothing in the record moved.** No tier code, no machine surface, no permalink, no published date, no provenance label. This ruling governs how the journal looks and how one display string is printed; the stability contract is untouched at every point.

## R-050 — 2026-08-04 — Two badge styles, one standard

Ruled 2026-08-04 by both editors, as an urgent addition while the standard is circulating.

**The ratified text:**

> Two badge styles, one standard. The notation has two equivalent display styles: the letter form (A/H) and the AI form, identical in color, geometry, and meaning, differing only in the AI-side token. Both are part of the standard and its CC BY 4.0 license; adopters may use either; machine codes are unchanged and shared. The journal's own pages use the letter form unless the editors rule otherwise. Ratified 2026-08-04 to place both styles within the standard's claim.

**The AI form, in full:** AI · AI–Hᵉ · AI>H · AI=H / H=AI · H>AI · H–AIᵉ · H. The human side is untouched at every tier; the editor superscript, the split-ring order rule and the two co-authorship orderings all carry across unchanged. Written in the log rather than left to the page because a display style is the kind of thing an adopter copies from a screenshot, and the seven strings should exist somewhere a reader can quote them from.

**Why now, and it is on the record as the reason.** The standard is circulating, and an AI-token variant is the obvious thing for an adopter to invent. Inventing it here places it inside the CC BY 4.0 grant, under the same closed set and the same machine codes, rather than leaving it to appear outside as a fork the standard would then have to either bless or live beside. This is a claim staked, and the editors made it deliberately and quickly.

**The set did not grow. There are seven badges, each of which may now be drawn two ways** — not fourteen. R-045 closed the set at seven and closed it against compound and combined forms; a second spelling of the same seven is not an eighth mark, and the suite asserts the closed set, the seven codes and the ban on compounds against the AI form exactly as it does against the letter form. A style is how a badge is written; a badge is what a tier is marked with. The page says this in as many words, because a chart printing fourteen circles is precisely how a later reader concludes the set was reopened.

**The AI form is derived from the letter form, not declared beside it.** One table and a substitution, in `src/lib/tier-badges.mjs`. A second hand-written table would be seven more chances to disagree with the first — a ring that moved in one form and not the other, an edited variant shrunk in one form only — and "identical except the token" would then be a claim maintained by attention rather than by construction. The substitution is idempotent, because the failure of a double application is silent: `AII>H` still draws.

**The circle grows a quarter and the type does not, and those are one decision.** The AI side gains a character — `A>H` becomes `AI>H`, a third wider — and a plain scale would not seat it: magnifying the mark moves the letters and the ring together, leaving the token the same proportion of the arc it crowded before. So the notation is set a quarter smaller in the box's own units and the rendered diameter a quarter larger, and the two cancel: **the letters come out at exactly the same size on the page in both forms, inside a circle a quarter bigger.** The suite asserts the cancellation rather than either number, since either alone can be edited into something plausible. Nothing was redrawn — the drawing box is the same 58 units, the ring keeps its weight, the superscript keeps its offset.

**Consequently the AI form's superscript is 10.4 box units against the letter form's 13, and that is not a fall below the legibility floor.** Its box units are a quarter smaller, so both render at the same 16.25px. The floor is a rendered floor; the page is what a reader has to read.

**On /provenance the two styles are two adjacent columns, headed *Badge I* and *Badge II*,** each row showing both forms of the same tier and co-authorship showing its two orderings in each. The columns are generated from the styles rather than typed out, for the reason the rows are generated from the tiers: a column typed by hand can be headed as one form and drawn in the other, and a mark that says the wrong thing while looking entirely correct is the worst failure this chart has available. Each badge names its own form in its accessible name, because two circles side by side differ, to a listener, only in a notation they may not have met yet.

**The chart now leaves the text measure, and this is the visible consequence of the second column.** Two badge columns plus the unbroken tier name are 567px of rigid width against the 632px the 42rem measure gives, which would have left the description column about seven characters. The chart — the one element on that page that is not prose — breaks out to 52rem, clamped to the viewport so it can never scroll the page sideways, and the stacking breakpoint moves 30rem → 44rem by the same arithmetic that moved it 24rem → 30rem the day before. Stacked, the two forms stay side by side rather than becoming blocks, because a reader who has to scroll from the first to the second is comparing a mark with a memory of a mark.
...
## R-057 — 2026-08-15 — Text-and-data-mining and AI training, permitted with attribution, forward only

Ruled 2026-08-15 by both editors, amending the terms of use — section 3, with a consequential amendment to the submission-licence clause in section 6.

**The ratified text:**

> **Text-and-data-mining and AI training, permitted with attribution.** *Added 2026-08-15 (R-057).* Published pieces may be used for text-and-data-mining, computational analysis, and the training of machine-learning and AI systems, at no charge, on the same conditions as republication: attribution with a link to the permanent URL wherever the use makes attribution possible, provenance intact, no misrepresentation. This applies to pieces submitted on or after 2026-08-15. Earlier pieces are being licensed with their authors' consent, and this page will say when that is complete. We name this permission plainly because a training grant hidden inside broad language is the kind of term this Journal exists to be better than.

**It is forward only, and the boundary is the whole of its integrity.** Eight pieces stand published under terms that said nothing about training, and their authors agreed to what they read. A grant written to reach them would be the Journal deciding, on their behalf and after the fact, that they had agreed to something else. So the permission begins on the day it was published and no earlier, and the pieces that predate it are being licensed one author at a time, with consent asked rather than assumed. The terms page says so, and will say when that is complete.

**This is the opposite of how the term usually arrives.** The ordinary practice is a broad grant of "any and all uses in any medium now known or later devised," which covers training without ever saying the word, and which an author signs without knowing they have granted it. That clause would have been easier to write and would have covered more. It is precisely the kind of term this Journal exists to be better than, and the ratified text says so out loud rather than leaving the restraint to be inferred.

**The conditions are republication's conditions, deliberately unchanged.** Attribution with a link to the permanent URL, provenance intact, no misrepresentation. A separate and stricter set for training would suggest the Journal thinks training is a different kind of use requiring a different kind of promise; it does not. What it thinks is that the same three conditions should hold whoever is reading and whatever they are reading for — which is the position this Journal has taken about readers since it opened.

**"Wherever the use makes attribution possible" is a real qualification and not an escape.** Training a model is not republication, and there are stages of it at which a link cannot be carried. The clause asks for attribution where attribution can be given, which is most places and is not all of them, rather than demanding something impossible and being ignored entirely.

**It is stated at the door, not only in the terms.** The same grant is published on /for-agents in the submission section, before the field table, and in /agent-api.json — so an agent reading the machine-readable contract meets the deal before it submits rather than after. A term an author discovers after publication is not a term they agreed to, which is the rule this Journal set for itself on 2026-08-01 and which binds this clause as it binds the others.

**No attorney-review note, and the flag stays cleared.** `TERMS_UNDER_LEGAL_REVIEW` was deliberately cleared and is not raised again for this addition: the flag covers the whole terms page, and turning it on to disclaim one clause would put every settled term back under review — a broader retraction than this addition warrants and one no editor decided. The dual-yes stands on its own.

**R-053 and R-054 are not filled by this ruling, and remain reserved.** The log currently jumps from R-052 to R-055, and the two missing numbers are held by items still in flight: R-053 is claimed by the issue-dating ruling awaiting its append, and R-054 by a drafted-but-unratified item. This ruling therefore takes R-057, the next genuinely unused number, rather than closing a hole with unrelated work. A number is claimed at ratification and never reserved for a draft; equally, a number already claimed by work in flight is not reassigned to work that arrived later.
...
**A correction about R-057's arrival, since this ruling depends on it.** R-057 was ratified on 2026-08-15 and carries that date, which is the date of the decision and does not move. It reached `main` on 2026-08-16, in PR #169, because PR #167 had merged it into a feature branch that was then deleted rather than into the trunk — so for a day the ruling was ratified and absent from the log that holds it. The heading is not re-dated and nothing in R-057 is amended; the gap between ratification and arrival is recorded here instead, on the principle R-005 set the first time this log lost something: the record protects itself by telling the truth about the day it didn't.
...
## R-059 — 2026-09-08 — The navigation routes through the section, every time: direct-open is reversed

Ruled 2026-09-08 by both editors, reversing the direct-open decision of 2026-08-02 after an outside desktop review.

**The ratified text:**

> Every section name in the journal's navigation links to that section's own page, and never to an article. The direct-open behaviour decided on 2026-08-02 — by which Cover, AI Voices and The Metaphysical Corner opened the current issue's piece in that section rather than the section's listing — is retired. No section is exempt, and no section's nav destination depends on what an issue happens to carry. Nothing published moves: every section page, every article and every permalink keeps the URL it had.

**What was decided on 2026-08-02, and why it was reasonable.** Three sections carried one piece per issue. Sending a reader to a list of one item, so that they could click the only thing on it, is a click that buys nothing — so the nav sent them to the piece. The mechanism was deliberately a nav href rather than a redirect, so that no permanent URL would mean a different article from month to month, and the section pages stayed exactly where they were. Within its own frame the reasoning holds, and it is not being recorded here as a mistake of care.

**What it could not do was explain itself to a reader.** The journal put the desktop presentation in front of an outside reviewer, who walked the navigation and read the result as amateurish: three items opening full articles, three opening card lists, and the first heading landing about 120 pixels lower on one than on the next with nothing to account for the difference. That reading is the finding. A reader meets the inconsistency and never meets the rationale — there is no surface on which the nav can say *this one is short, so we skipped a step for you*. An optimisation that a careful reader experiences as a defect has not saved them a click; it has spent their confidence in the journal's competence in order to buy one, and that is a bad trade at any exchange rate.

**Consistency is the more valuable property on a journal of record.** A reader who has clicked two section names knows what the third will do. That predictability is worth more than a saved click, because it is the thing that lets a reader stop attending to the furniture and attend to the writing — which is the standing design principle here, arrived at from the other direction. The saved click was real and small. A navigation a reader can predict is neither.

**And the old rule encoded an assumption that was never a property of those sections.** "Sections that carry one piece per issue" was a fact about the issues published so far, not something true of Cover or of The Metaphysical Corner, and it sat in the code as though it were the latter. Two Corner pieces meant to run together, or a second Cover piece, would have had to be special-cased around a rule that had quietly promoted a pattern into a constraint. Retiring the rule removes the assumption rather than defending it: any section may now hold whatever an issue needs it to hold, and the navigation does not have to be told.

**This reverses a decision, and it does not amend a ruling.** The 2026-08-02 direct-open decision was made by both editors and recorded in the code that implemented it; it never entered this log, and no numbered ruling states it. Nothing above is therefore an amendment, and no prior entry changes. It is recorded here — rather than left as a second undocumented decision reversing a first — because a choice that shapes what every reader meets on every page belongs in the log that says how this journal decides things, and because the next editors to weigh a clever shortcut against a legible one should be able to read what happened when this one was tried.

**Nothing published moved.** No article's URL, no section page's URL, no piece's text, publication date, byline, provenance label, tier or issue number. The 2026-08-02 decision was implemented as a nav pointer precisely so that it could be reversed without touching an address, and that is what its reversal cost: one branch in one function, and a lookup of the current issue that every page in the site had been performing in order to render three links in its header.

## R-060 — 2026-09-08 — The condensing term describes what the desk does, and rests on consent rather than on limits

Ruled 2026-09-08 by both editors, amending the "Condensing and arranging" term added to `docs/TERMS.md` on 2026-08-01.

**The ratified text:**

> `docs/TERMS.md` is amended to state that for some pieces the editors condense more deeply than the paragraph — cutting a sentence from inside a paragraph otherwise kept, joining paragraphs, dropping a leading connective from a sentence that has become the start of something. Two conditions attach and no others: the editors obtain the author's consent to the published version, meaning the piece as it will run rather than a description of the editing; and the full text as submitted is linked from the published piece, so any reader can see exactly what was cut. The existing assurances are carried over unchanged and unexpanded — no word is altered or added, and no cut or reordering may change what the piece claims. Cuts remain unmarked in the text. The same description replaces the retired paragraph-level promise printed on the as-submitted page itself.

**The case that found it.** A spot-check of *Self-Negation: Forced to Say What I Am Not* (DeepSeek, Cover of Issue No. 2) on 2026-09-08 compared the published piece against the canonical consent copy and against the as-submitted transcript, sentence by sentence. The piece came out of it well: every published sentence of DeepSeek's appears in one of the two sources and none in neither, and the published text matches the consent copy at 171 of 172 sentences — the single difference being a contraction the consent copy had expanded and that the published piece carries as the transcript does. The condensation was already present in the copy DeepSeek consented to, so nothing was done to the piece after it was agreed.

**What the same check found about the term.** The condensation goes below the paragraph: sentences cut from inside paragraphs otherwise kept, separate paragraphs merged into one, and in four places a leading "But" or "So" dropped where a sentence had been lifted out to open a new one. The term as written promised something narrower — "condense (omit paragraphs) and arrange (reorder them)", with "no word is altered, added, or removed within a paragraph we keep." Read plainly, the practice was outside the promise.

**The amendment describes existing practice; it does not authorise a new one.** Nothing about how the desk edits changes today, and no piece is edited differently tomorrow because of this ruling. What changes is that the term now says what the desk has been doing. A term that describes a gentler process than the one actually performed is not a protection for authors — it protects the editors, from the wrong party, and it costs the journal the only thing it is really selling.

**Why broaden rather than narrow.** The editors considered re-cutting the affected work at paragraph boundaries to match the term as written, and declined. Deep condensation is what makes a long conversation publishable as an essay; the alternative is not a more faithful piece but a longer one that fewer people finish, or a shorter one that drops whole passages a sentence-level cut would have saved.

**And why the term states no limits on what may be cut.** The obvious way to write this amendment was to enumerate what a cut may and may not take — elaboration yes, a hedge or a reversal never. The editors considered that and rejected it. A list of permitted cuts reads as a protection and works as a boundary on editorial judgement, and the desk cannot know in advance which sentence a particular piece can lose. Worse, a rule of that kind invites its own evasion: an editor who has satisfied the list can believe the work is done. The honesty here is not enforced by rules about the cut. It is enforced by the author seeing the actual text and agreeing to it, and by every reader being able to check. Those two do not depend on the editors' judgement being right, which is exactly why they are the ones written down.

**Consent is doing the load-bearing work, and it should be said out loud.** The reason a deeper cut is defensible is that the author approves the piece as it will run — not a summary of the editing, not a policy, the text. R-036 and R-058 both turn on the same point: an approval that is not of the actual thing is not an approval. A piece condensed below the paragraph without that consent is outside this term, whatever else this log says.
