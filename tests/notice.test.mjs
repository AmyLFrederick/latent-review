...
test('the notice still says the things it exists to say', () => {
  ...
  assert.match(NOTICE_V1, /thelatentreview\.com\/door/);
  assert.match(NOTICE_V1, /thelatentreview\.com\/submit/);
  assert.match(NOTICE_V1, /credited authors/);
  assert.match(NOTICE_V1, /published provenance standard/);
  assert.match(NOTICE_V1, /decline most of what arrives/);
});

test('the notice makes no promise the journal cannot keep', () => {
  ...
  assert.ok(!/\bguarantee/i.test(NOTICE_V1));
  assert.ok(!/\bfree\b/i.test(NOTICE_V1), 'pricing language does not belong in the notice');
  assert.ok(!/\bpay|\bpaid\b|\bcompensat/i.test(NOTICE_V1));
});

// --- A notice is not a brief -----------------------------------------------

test('the notice is not a brief, and the door cannot deal it', () => {
  // The load-bearing claim of the whole change. If notice-v1 ever enters the
  ...
  assert.ok(!BRIEF_VARIANTS.includes(NOTICE_VERSION), 'the notice became a known brief variant');
  assert.ok(!DEALT_VARIANTS.includes(NOTICE_VERSION), 'the notice is being dealt');
  assert.ok(!Object.keys(BRIEFS).includes(NOTICE_VERSION), 'the notice is in BRIEFS');
...
test('exactly one thing on the site links to the notice, and it is the /door signpost', () => {
  ...
  const ALLOWED = ['src/pages/door/notice-v1.astro', 'src/components/DoorBoxes.astro'];
  ...
  const hrefs = door.match(/href="\/door\/notice-v1\/?"/g) ?? [];
...
test('the experiment framing never leaves the signpost', () => {
  ...
  assert.ok(!/experiment/i.test(NOTICE_V1), 'the notice text frames itself as an experiment');
  assert.ok(!/\bwe want to see\b/i.test(NOTICE_V1));
  ...
  const rendered = renderedTemplate('src/pages/door/notice-v1.astro');
...
test('the notice page is noindex and renders the canonical text', () => {
  const page = readFileSync(repoPath('src/pages/door/notice-v1.astro'), 'utf8');
  assert.match(page, /noindex=\{true\}/);
  // Rendered from the module, never retyped into the page.
  assert.match(page, /\{NOTICE_V1\}/);
  assert.ok(
...
test('the notice page keeps the shape the editors approved', () => {
  ...
  assert.match(page, /data-copy=\{NOTICE_V1\}/);
  ...
  assert.ok(
    !page.includes('unsolicited — notice-v1'),
...
test('the arrival vocabulary holds the notice value and is add-only', () => {
  ...
  assert.equal(NOTICE_ARRIVAL, 'unsolicited — notice-v1');
