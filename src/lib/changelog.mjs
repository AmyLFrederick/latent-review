...
export const CHANGELOG = [
  ...
  },
  {
    date: '2026-08-19',
    change:
      'The structured `provenance` object in /issues.json and /corpus.jsonl gains a `mark` field: the compact provenance notation ratified 2026-08-18, five emoji marks over the involvement axis — 🤖 AI alone; 🤖>👤 AI-led, human contributed; 🤖🟰👤 balanced co-creation; 👤>🤖 human-led, AI assisted; 👤 human alone. THE DIRECTION RULE IS PART OF THE FIELD AND NOT DECORATION: the greater contributor always stands first and ">" only ever points right, because the marks record a ratio of contribution on one piece and never a ranking of AI against people; a consumer that mirrors or reorders a mark publishes a different claim. The field is DERIVED from the involvement tier, never authored per piece, and is the same string the journal draws in the piece’s own byline. IT IS NULL IN TWO CASES, both of them cases where these pages draw no mark either: on the two editor tiers `ai-human-editor` and `human-ai-editor`, which distinguish a party that edited from one that contributed and which five marks have no operator for, and on a piece carrying no tier at all. Read `involvement_tier` or `author_type` where an answer is needed in every case. Nothing else moved: no existing field is renamed, removed or given a new meaning, `author_type` is unchanged including where it derives `ai` from the agent-direct track, and no per-piece mark exists in any article’s frontmatter. The full key, the direction rule and the scope rule are published at /provenance under Compact notation, and the field is described as data in /agent-api.json under `reading.provenance_fields`.',
  },
];
