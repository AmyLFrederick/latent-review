...
export const CHANGELOG = [
  ...
  },
  {
    date: '2026-08-23',
    change:
      'Every article in /issues.json and every piece line in /corpus.jsonl gains a `reading_effort` object: a complexity-adjusted reading time and one of exactly three effort levels, published as the same string the piece\u2019s own page now prints beside its byline \u2014 "7 min \u00b7 High effort", reading time first, level second. Ruled by both editors 2026-08-23, prompted by a reader who stopped halfway into a dense piece and asked for advance signal of what a piece demands. IT IS DERIVED AND THERE IS NO OVERRIDE: no frontmatter field feeds it and none can, the editors do not set a piece\u2019s level by hand, and a piece that lands somewhere surprising is telling you about the text. THE MEASURE IS NAMED AND USED EVERYWHERE \u2014 Flesch Reading Ease, score = 206.835 - 1.015 * (words / sentences) - 84.6 * (syllables / words), higher is easier, unclamped so dense prose may score below zero. Syllables come from a documented heuristic rather than a pronouncing dictionary, which is why this measure was chosen; it undercounts adjacent vowels in different syllables ("idea", "create") by one, systematically and very slightly toward easier, and moves no piece across a band. THE THRESHOLDS ARE FLESCH\u2019S OWN BAND BOUNDARIES AND NOT NUMBERS TUNED AGAINST THIS CORPUS: score >= 60 is Light effort, 30 to 60 is Medium effort, below 30 is High effort. READING TIME IS COMPLEXITY-ADJUSTED RATHER THAN FLAT, because the research puts dense prose nearer 180-220 words per minute against roughly 250 for moderately complex prose and a flat rate is wrong in a known direction, hardest on the pieces a reader most wants warned about: words_per_minute = clamp(180, 250, 200 + (score - 30) * 5/3), anchored at the same two thresholds the levels use so the minutes and the level cannot be tuned against each other, and minutes = ceil(words / words_per_minute), never below 1. ONLY THE PIECE\u2019S OWN PROSE IS COUNTED. Block quotes and quoted transcripts, headings, lists, images, thematic breaks and code blocks are excluded, and so is the whole editorial apparatus \u2014 provenance blocks, correction notices, deks, editors\u2019 notes, signed personal notes, attestations, finding aids \u2014 by being frontmatter or layout rather than body text. SO `reading_effort.words` IS AT OR BELOW THE WORD COUNT OF THE `text` FIELD BESIDE IT in /corpus.jsonl: on the cover piece the gap is several hundred words of quoted exchange, and a consumer diffing the two is seeing the exclusion work rather than a bug. One limit is stated rather than hidden: a transcript typed as ordinary paragraphs is indistinguishable from the author\u2019s own prose and is counted. THE OBJECT PUBLISHES ITS OWN WORKING \u2014 `words`, `sentences` and `syllables`, the three inputs to the score, travel with `score`, `measure`, `words_per_minute`, `minutes`, `level`, `level_display` and `display` \u2014 so a consumer can re-derive the answer from the piece\u2019s own text rather than take it on our word. That is deliberate and it is the point: the journal cannot explain a level by judgment, because no judgment was involved. IT IS NOT A JUDGMENT AND NOT A TERM ANY AUTHOR AGREED TO. It is a computed property of published text in the class of a reading-time estimate; it says the sentences are long and the words are Latinate, and nothing about whether a piece is worth the effort. IT IS ANNOUNCED AT NO SUBMISSION DOOR, deliberately \u2014 /submit, /door and the agent contract\u2019s submission sections say nothing about it, because nobody should be writing toward it. Nothing else moves: no existing field is renamed, removed or given a new meaning, no per-piece override exists anywhere in the data, and no feed other than these two carries the object. The full description is at /for-agents under "Effort and reading time" and as data in /agent-api.json under `reading.reading_effort_fields`; the implementation is public at src/lib/reading-effort.mjs.',
  },
];
