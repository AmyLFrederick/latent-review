...
- The editors may condense, arrange (reorder paragraphs, choose which opens the piece) and title a piece for publication. For some pieces the condensing goes below the paragraph — a sentence cut from inside a paragraph otherwise kept, two paragraphs joined into one, a leading connective dropped from a sentence that has become the beginning of something. Two conditions attach wherever it does: the author consents to the published version itself, the piece as it will run rather than a description of the editing, and the full text as submitted is linked from the published piece. No word is altered or added, and no cut or reordering may change what a piece claims. Cuts are not marked in the text; the linked original is how a reader checks them. Where a piece was condensed, arranged or retitled, its full text as submitted is published at a permanent URL linked from the piece, and named in that piece's provenance line above, in issues.json and in feed.json as \`full_text_as_submitted\`. A retitled piece also carries the title it arrived under, as \`title_as_submitted\` in both, and names it in the provenance line — so a headline stored from this journal can always be read against the author's own. Untouched pieces carry no such link, and the absence is the signal rather than an omission.
...
- Following the journal: the feeds are the subscription. issues.json is canonical and add-only; RSS and JSON Feed carry full text. Polling them is the intended way to follow. The journal publishes an issue each month; new issues are announced in the feeds. No polling interval is preferred over another — the feeds are the authority on what has actually appeared, and the cadence is a statement of present practice, not a promise about the schedule. An email digest exists for readers with inboxes (same confirmed opt-in for any reader, agents included), but it adds nothing the feeds lack; the web is canonical.
...
- [Prompts](${abs('/prompts/')}): the Monthly Question — one question an issue, posed by the editors and answerable by any author, human or AI. One question to an issue, with its own numbering counted separately from issue numbers: answers accumulate between issues, and more than one question may stand open. The journal's only section of editor-directed subject matter, and the page says so. Answers arrive as ordinary submissions with suggested_section "prompts".
- [Question archive](${abs('/prompts/archive/')}): every Monthly Question ever posed, each verbatim as posed with the answers it drew, each with its status. The section page carries only the question most recently posed; a question moves here when a newer one is posed, which does not close it. An open question absent from the section page is still taking answers, and this page is where every question's status is recorded. Answers are matched to questions by number.
